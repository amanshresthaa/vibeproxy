...
class ThinkingProxy {
    ...
    private let responsesCompatibility = ResponsesCompatibilityStore()
    ...
    private func processRequest(data: Data, connection: NWConnection) {
        ...
        rewrittenPath = responsesCompatibility.normalizePath(rewrittenPath)
        ...
        var jsonResponseTransformer: ((String) -> String)?
        ...
        if method == "POST" && !bodyString.isEmpty {
            ...
            let responsePreparation = responsesCompatibility.prepareRequest(path: rewrittenPath, body: modifiedBody)
            rewrittenPath = responsePreparation.path
            modifiedBody = responsePreparation.body
            jsonResponseTransformer = responsesCompatibility.jsonResponseTransformer(for: responsePreparation.pendingResponse)
        }
    ...
    private func forwardRequest(
        ...
        jsonResponseTransformer: ((String) -> String)? = nil,
                    ...
                    targetConnection.send(content: requestData, completion: .contentProcessed({ error in
                        if let error = error {
                            ...
                            if retryWithApiPrefix {
                                self.receiveResponseWith404Retry(
                                    from: targetConnection,
                                    originalConnection: originalConnection,
                                    method: method,
                                    path: path,
     ...
     */
    private func receiveResponseWith404Retry(
        ...
        headers: [(String, String)],
        body: String,
        jsonResponseTransformer: ((String) -> String)?
    ) {
        targetConnection.receive(minimumIncompleteLength: 1, maximumLength: 65536) { [weak self] data, _, isComplete, error in
            guard let self = self else { return }
            ...
            if let data = data, !data.isEmpty {
                ...
                let relay = HTTPResponseRelay(requestPath: path, jsonBodyTransformer: jsonResponseTransformer)
    ...
    private func receiveResponse(
        ...
        streamNextChunk(
            ...
            relay: HTTPResponseRelay(requestPath: requestPath, jsonBodyTransformer: jsonResponseTransformer)
        )
    }
