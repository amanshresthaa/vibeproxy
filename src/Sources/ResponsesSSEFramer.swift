...
final class HTTPResponseRelay {
    private enum BodyMode {
        ...
        case bufferedJSON
        case chunkedBufferedJSON
    }

    private let requestPath: String
    private let jsonBodyTransformer: ((String) -> String)?
    ...
    init(requestPath: String, jsonBodyTransformer: ((String) -> String)? = nil) {
        self.requestPath = requestPath
        self.jsonBodyTransformer = jsonBodyTransformer
    }
    ...
    func process(_ data: Data, isComplete: Bool) -> [Data] {
        ...
        let (headerData, resolvedBodyMode) = rewriteHeaders(Data(headerBuffer.prefix(headerEnd)))
    ...
    private func processBody(_ data: Data, isComplete: Bool) -> [Data] {
        ...
        case .bufferedJSON:
            ...
            defer { bufferedBody.removeAll(keepingCapacity: true) }
            return finalizedJSONBodyOutput()
        case .chunkedBufferedJSON:
            for decodedChunk in chunkedDecoder.decode(data, isComplete: isComplete) {
                bufferedBody.append(decodedChunk)
            }
            guard isComplete else { return [] }
            defer { bufferedBody.removeAll(keepingCapacity: true) }
            return finalizedJSONBodyOutput()
    ...
    private func finalizedJSONBodyOutput() -> [Data] {
        guard !bufferedBody.isEmpty else { return [] }
        guard let bodyString = String(data: bufferedBody, encoding: .utf8) else {
            return [bufferedBody]
        }

        let transformedBody = jsonBodyTransformer?(bodyString) ?? bodyString
        return [Data(transformedBody.utf8)]
    }

    private func rewriteHeaders(_ headerData: Data) -> (Data, BodyMode) {
        ...
        let hasJSONContentType = rawLines.contains {
            $0.lowercased().hasPrefix("content-type:") && $0.lowercased().contains("application/json")
        }
        ...
        for line in rawLines.dropFirst() {
            ...
            if lowercasedLine.hasPrefix("transfer-encoding:") || lowercasedLine.hasPrefix("content-length:") {
                guard hasEventStream || (hasJSONContentType && shouldBufferJSON) else {
        ...
        if !hasEventStream && !(hasJSONContentType && shouldBufferJSON) {
            return (headerData, .passthrough)
        }
        ...
        if hasEventStream {
            bodyMode = hasChunkedTransferEncoding ? .chunkedResponsesSSE : .responsesSSE
        } else {
            bodyMode = hasChunkedTransferEncoding ? .chunkedBufferedJSON : .bufferedJSON
        }
        return (rewrittenHeaderData, bodyMode)
    }

    private var shouldBufferJSON: Bool {
        requestPath.split(separator: "?", maxSplits: 1, omittingEmptySubsequences: false).first == "/v1/responses" &&
        jsonBodyTransformer != nil
    }
}
