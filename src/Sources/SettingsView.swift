    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Toggle(isOn: $serverManager.forceFastServiceTier) {
                Text("Request fast tier for GPT requests")
                    .font(.caption)
            }
            .toggleStyle(.checkbox)
            .help("When enabled, VibeProxy requests Codex fast-mode service_tier for supported GPT and o-series Responses API requests when the client did not already specify a service tier. Actual tiering is determined downstream by the bundled backend and provider.")
        }
        .padding(.leading, 28)
        .padding(.top, 4)
    }
